...
const duplicate = copy(originalFlavors);
...
/* I just used/returned the index[position] so I could pass in a number for the parameter. I can now search by index position instead of index value. */

function getFlavorByIndex(arr, desiredIndex){
    return arr[desiredIndex];
    console.log(arr[desiredIndex]);
}
...
/* I am using the array and string I want to search with as parameters */
function removeFlavorByName(arr, string){
    /* I am using a for loop to search throught the entire array (all indexes). I use .splice method to remove specific flavors form the array */
    for (let i = 0; i < originalFlavors.length; i++) {
        /* I set a condition of the array index array[i] to be met with the string parameter. */
      if (arr[i] === string) {
         arr.splice(i, 1);
      }
    }
